...
<xsl:stylesheet version="1.0"
                ...
                exclude-result-prefixes="encoder i18n mcrxml utilities xalan xsl">
  ...
  <xsl:template match="head">
    <head>
      ...
      <script type="text/javascript" src="{$WebApplicationBaseURL}webjars/jquery/{$UBO.Frontend.jquery.version}/jquery.min.js"/>
      <script type="text/javascript" src="{$WebApplicationBaseURL}webjars/bootstrap/{$UBO.Frontend.bootstrap.version}/js/bootstrap.bundle.min.js"/>
  ...
  <xsl:template name="layout.login">
    ...
    <div class="nav-item me-2">
      ...
      <span class="text-primary ms-2 me-1">
        <xsl:value-of select="'|'"/>
      </span>
    </div>
  ...
  <xsl:template name="layout.imprintline">
    <div class="imprintlinewrapper">
      ...
      <xsl:for-each select="$navigation.tree/item[@role='meta']/item">
        <span>
          <a>
            <xsl:attribute name="href">
              <xsl:choose>
