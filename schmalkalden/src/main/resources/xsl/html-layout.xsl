...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="feed-title"
                select="concat(i18n:translate('thunibib.university.full.name'), ', ', i18n:translate('ubo.publications'))"/>
  ...
  <xsl:template match="head">
    <head>
      ...
      <link rel="alternate" type="application/rss+xml" title="{$feed-title} - RSS-Feed"
            href="{$WebApplicationBaseURL}servlets/solr/feed?XSL.Style=rssfeed"/>
      <link rel="alternate" type="application/atom+xml" title="{$feed-title} - Atom-Feed"
            href="{$WebApplicationBaseURL}servlets/solr/feed?XSL.Style=atomfeed"/>
      ...
      <script
        src="{$WebApplicationBaseURL}webjars/bootstrap-select/{$UBO.Frontend.bootstrap-select.version}/js/bootstrap-select.min.js"/>
      <link
        href="{$WebApplicationBaseURL}webjars/bootstrap-select/{$UBO.Frontend.bootstrap-select.version}/css/bootstrap-select.min.css"
        rel="stylesheet"/>
            ...
            <xsl:variable name="href">
              <xsl:choose>
                <xsl:when test="$CurrentLang = 'de'">
                  <xsl:value-of select="'https://www.hs-schmalkalden.de/'"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="'https://www.hs-schmalkalden.de/en/'"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:variable>
  ...
  <xsl:template name="layout.footer">
    <footer>
      <div class="container">
        <div class="row">
          <div class="col cd-line"/>
        </div>
      </div>
      <div class="container bg-secondary pt-2 pb-2">
        <div class="row">
