...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="lst/arr[@name='name_id_type,name_id_type']">
    <xsl:if test="not(mcrxsl:isCurrentUserGuestUser())">
      ...
      <div class="mb-3 thunibib-author-identifier-statistic-container {$ChartsCommon.border.class}">
        <article class="card">
          <div class="card-body">
            <h3>
              <xsl:value-of select="i18n:translate('thunibib.statistic.aut.ids.used')"/>
            </h3>
