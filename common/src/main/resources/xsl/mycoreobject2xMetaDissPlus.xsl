...
<xsl:stylesheet 
  ...
  <xsl:template name="repositoryPublisher">
    <xsl:choose>
      <xsl:when test="$mods/mods:name[mods:role/mods:roleTerm/text()='his' and @valueURI]">
        ...
        <xsl:variable name="myURI" select="concat('classification:metadata:0:parents:ORIGIN:',$insti)" />
  ...
  <xsl:template name="license">
    ...
    <xsl:for-each select="$mods/mods:accessCondition[@type='use and reproduction' and @xlink:href]">
      ...
      <xsl:variable name="license" select="document(concat('classification:metadata:0:children:licenses:',$licenseId))"/>
